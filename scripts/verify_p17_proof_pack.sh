...
echo '[1/5] Python proof scripts compile'
python -m py_compile \
...
echo '[2/5] Cloud proof scripts default to dry-run'
...
echo '[3/5] Model Armor setup defaults to dry-run'
bash "$ROOT/deployment/gcp/setup_model_armor_demo.sh" | grep -q 'DRY_RUN'

echo '[4/5] IBM ground-truth separation smoke test'
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
cat > "$TMP/ibm.csv" <<'CSV'
...
echo '[5/5] Moov/NACHA 94-char fixture validation smoke test'
