...
class FeatureContributionItem(BaseModel):
    ...
    description: str = Field(
        ..., description="Human-readable explanation of this feature's contribution"
    )
...
class ReturnRiskContextEnvelope(BaseModel):
    ...
    tenant_scope: str = Field(
        ..., min_length=1, description="Authenticated tenant scope identifier"
    )
    return_event_ref: str = Field(
        ..., min_length=1, description="Unique return event identifier (e.g. RET-EVT-2026-001)"
    )
    return_code: str = Field(
        ...,
        min_length=3,
        max_length=4,
        description="Standard NACHA return reason code (e.g. R01, R08, R10)",
    )
    return_code_label: str = Field(
        ..., min_length=1, description="Standard NACHA return code label (e.g. INSUFFICIENT_FUNDS)"
    )
    risk_score: float = Field(
        ..., ge=0.0, le=100.0, description="Authoritative deterministic risk score (0-100)"
    )
    ...
    historical_summary: Optional[str] = Field(
        None, description="Summary of past return patterns and rate benchmarks"
    )
    sla_cutoff_context: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="SLA window and cutoff timing metadata"
    )
    workflow_id: Optional[str] = Field(default=None, description="Associated workflow identifier")
    incident_id: Optional[int] = Field(
        default=None, description="Associated incident ID if triggered from an incident"
    )
    correlation_id: Optional[str] = Field(
        default=None, description="Distributed correlation trace ID"
    )
...
class ReturnRiskAssessment(BaseModel):
    ...
    principal_drivers: List[str] = Field(
        default_factory=list,
        description="Ranked list of principal feature drivers contributing to risk",
    )
    ...
    evidence_refs: List[str] = Field(
        default_factory=list,
        description="Authorized evidence citations (RET-EVT-*, INCIDENT-*, EVID-*)",
    )
    memory_refs: List[str] = Field(
        default_factory=list,
        description="Authorized advisory memory citations (MEM-HIT-*, MEM-PROFILE-*)",
    )
