...
from armor.config import GuardrailDecision, GuardrailMode
...
class GuardrailResult(BaseModel):
    ...
    decision: GuardrailDecision = Field(..., description="High-level guardrail action: ALLOW, BLOCK, SANITIZED, ERROR")
    verdict: ArmorVerdict = Field(default=ArmorVerdict.ALLOWED, description="Fine-grained screening verdict")
    transformed_text: Optional[str] = Field(default=None, description="Sanitized or de-identified text if applicable")
    violations: List[str] = Field(default_factory=list, description="List of detected policy or filter violation codes")
    reason: Optional[str] = Field(default=None, description="Human-readable explanation of the verdict")
    pii_detected: bool = Field(default=False, description="Whether sensitive PII/SDP data was detected")
    injection_detected: bool = Field(default=False, description="Whether prompt injection or jailbreak was detected")
    confidence: float = Field(default=1.0, ge=0.0, le=1.0, description="Detection confidence score")
    latency_ms: float = Field(default=0.0, description="Screening roundtrip latency in milliseconds")
    ...
    raw_metadata: Optional[Dict[str, Any]] = Field(default=None, description="Raw response payload from provider")
